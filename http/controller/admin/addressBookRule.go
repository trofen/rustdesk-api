...
func (abr *AddressBookRule) Detail(c *gin.Context) {
	id := c.Param("id")
	iid, _ := strconv.Atoi(id)
	t := service.AllService.AddressBookService.AddressBookRuleInfoById(uint(iid))
	if t.Id > 0 {
...
func (abr *AddressBookRule) CheckForm(t *model.AddressBookRule) (string, bool) {
	if t.UserId == 0 || t.CollectionId == 0 {
	...
	if ab.UserId != t.UserId || ab.CollectionId != t.CollectionId || ab.CollectionId == 0 {
		return "ParamsError", false
	}
...
func (abr *AddressBookRule) Update(c *gin.Context) {
	...
	if ex := service.AllService.AddressBookService.AddressBookRuleInfoById(f.Id); ex.Id == 0 {
		response.Fail(c, 101, response.TranslateMsg(c, "ItemNotFound"))
		return
	}
	msg, ok := abr.CheckForm(f)
